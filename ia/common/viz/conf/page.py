...
def embed_jira_macro(
    jql_query,
    columns="key,summary,type,assignee,reporter,priority,status,issuelinks,epic name,sprint",
    maximum_issues=20,
):
    macro_id = str(uuid.uuid4())
...
class PageBuilder:
    ...
    def get_content(self, space_key, title, expand=["version", "content"]):
        pages = self.cli.get_content(
            space_key=space_key, title=title, expand=["version", "content"]
        )
        for p in pages:
            result = p  # take just first element from the generator
            page = self.cli.get_content_by_id(
                result.id,
                expand=[
                    "body.storage",
                    "body.editor",
    ...
    def create_or_update(
        self, space_key, title, new_content, attachments=[], parent_page_title=None
    ):
        """
